const baseCoursePath = "/generative-ai-for-beginners-main/";
console.log("%c Microsoft Generative Ai For Beginners | Official Microsoft Curriculum ", "background: #00f2ff; color: #0a0b10; font-weight: bold; padding: 4px; border-radius: 4px;");

const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.protocol === 'file:';

let courseData = [];
let progress = JSON.parse(localStorage.getItem('course_progress_generative_ai_for_beginners')) || {};
...
async function loadData() {
    try {
        ...
        const response = await fetch(`/generative-ai-for-beginners-main/lessons_data.json?v=${new Date().getTime()}`);
...
function toggleComplete(lessonId) {
    ...
    localStorage.setItem('course_progress_generative_ai_for_beginners', JSON.stringify(progress));
    renderDashboard();
    updateOverallProgress();
}
