const baseCoursePath = "/ai-agents-for-beginners-main/";
console.log("%c Microsoft Ai Agents For Beginners | Official Microsoft Curriculum ", "background: #00f2ff; color: #0a0b10; font-weight: bold; padding: 4px; border-radius: 4px;");

const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.protocol === 'file:';

let courseData = [];
let progress = JSON.parse(localStorage.getItem('course_progress_ai_agents_for_beginners')) || {};
...
async function loadData() {
    try {
        ...
        const response = await fetch(`/ai-agents-for-beginners-main/lessons_data.json?v=${new Date().getTime()}`);
...
function toggleComplete(lessonId) {
    ...
    localStorage.setItem('course_progress_ai_agents_for_beginners', JSON.stringify(progress));
    renderDashboard();
    updateOverallProgress();
}
...
function openNotebook(lessonPath, notebookName, lessonId) {
    const githubRepo = "microsoft/ai-agents-for-beginners";
    const branch = "main";
