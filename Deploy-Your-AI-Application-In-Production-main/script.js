const baseCoursePath = "/Deploy-Your-AI-Application-In-Production-main/";
console.log("%c Microsoft Deploy Your Ai Application In Production | Official Microsoft Curriculum ", "background: #00f2ff; color: #0a0b10; font-weight: bold; padding: 4px; border-radius: 4px;");

const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.protocol === 'file:';

let courseData = [];
let progress = JSON.parse(localStorage.getItem('course_progress_Deploy_Your_AI_Application_In_Production')) || {};
...
async function loadData() {
    try {
        ...
        const response = await fetch(`/Deploy-Your-AI-Application-In-Production-main/lessons_data.json?v=${new Date().getTime()}`);
...
function renderDashboard() {
    ...
    courseData.forEach((section, sIdx) => {
        ...
        if (sidebarNav) sidebarNav.appendChild(navSection);
    });
}
...
function toggleComplete(lessonId) {
    ...
    localStorage.setItem('course_progress_Deploy_Your_AI_Application_In_Production', JSON.stringify(progress));
    renderDashboard();
    updateOverallProgress();
}
